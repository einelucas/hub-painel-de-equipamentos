"""Somente leitura: lista os valores reais de `work_need_status_observed`
("Status Necessidade Obra" do Monday) staged para os 41 equipamentos do C2."""
...
async def main() -> None:
    async with SessionLocal() as session:
        ...
        for record in records:
            value = record.normalized_payload.get("work_need_status_observed")
            key = repr(value)
            counter[key] += 1
