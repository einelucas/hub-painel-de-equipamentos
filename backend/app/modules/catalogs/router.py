...
async def get_units(
    ...
    return CatalogListOut(items=[UnitOut.model_validate(item) for item in await service.list_units(session)])
...
async def get_project_contexts(
    ...
    items = await service.list_project_contexts(session, unit_id)
...
async def get_areas(
    ...
    return CatalogListOut(
        items=[AreaOut.model_validate(item) for item in await service.list_areas(session, unit_id)]
    )
...
async def get_work_packages(
    ...
    items = await service.list_work_packages(session, project_context_id)
